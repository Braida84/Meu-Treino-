...
const defaultEquipmentCatalog=[
{id:'treadmill',name:'Esteira',category:'Cardio',notes:'',available:true,system:true},
{id:'leg_extension',name:'Cadeira extensora',category:'Musculação',notes:'',available:true,system:true},
{id:'leg_curl',name:'Cadeira flexora',category:'Musculação',notes:'',available:true,system:true},
{id:'ms8000',name:'Estação Uplift MS-8000',category:'Musculação',notes:'Torre e polias',available:true,system:true},
{id:'dumbbells',name:'Halteres',category:'Peso livre',notes:'Diversos pesos',available:true,system:true},
{id:'mat',name:'Colchonete',category:'Acessório',notes:'',available:true,system:true},
{id:'pilates_ball',name:'Bola de Pilates',category:'Acessório',notes:'',available:true,system:true},
{id:'bench',name:'Banco ajustável',category:'Musculação',notes:'',available:true,system:true},
{id:'barbell',name:'Barra e suporte',category:'Peso livre',notes:'',available:true,system:true},
{id:'plates',name:'Anilhas',category:'Peso livre',notes:'',available:true,system:true},
{id:'pullup_bar',name:'Barra fixa',category:'Musculação',notes:'',available:true,system:true},
{id:'ankle_weights',name:'Pesos de areia',category:'Acessório',notes:'',available:true,system:true},
{id:'jump_rope',name:'Corda de pular',category:'Cardio',notes:'',available:true,system:true},
{id:'step',name:'Degrau 20 cm',category:'Acessório',notes:'',available:true,system:true},
{id:'ab_roller',name:'Rolo abdominal',category:'Acessório',notes:'',available:true,system:true}
];
function getEquipmentCatalog(){
 const saved=load('equipmentCatalogV4',null);
 if(Array.isArray(saved)&&saved.length)return saved;
 const legacy=load('equipment',null);let base=JSON.parse(JSON.stringify(defaultEquipmentCatalog));
 if(Array.isArray(legacy)){const on=new Set(legacy);base=base.map(e=>({...e,available:on.has(e.id)}))}
 store('equipmentCatalogV4',base);return base;
}
function saveEquipmentCatalog(list){store('equipmentCatalogV4',list)}
...
let exerciseOverrides={},currentSubIndex=null;
function selectedEquipment(){return getEquipmentCatalog().filter(e=>e.available).map(e=>e.id)}
function renderEquipment(){renderEquipmentManager()}
function renderEquipmentManager(){
 const host=$('equipmentManager');if(!host)return;const list=getEquipmentCatalog();
 host.innerHTML=list.map(e=>`<div class="equipment-card"><div class="eqtop"><div><b>${e.name}</b><div class="eqmeta"><span class="badge">${e.category||'Outro'}</span><span class="badge ${e.available?'':'off'}">${e.available?'Disponível':'Indisponível'}</span></div>${e.notes?`<div class="eqmeta">${e.notes}</div>`:''}</div><div class="eqactions"><button onclick="toggleEquipment('${e.id}',${!e.available})">${e.available?'Desativar':'Ativar'}</button><button onclick="openEquipmentModal('${e.id}')">Editar</button><button class="danger" onclick="deleteEquipment('${e.id}')">Excluir</button></div></div></div>`).join('');
}
function toggleEquipment(id,on){saveEquipmentCatalog(getEquipmentCatalog().map(e=>e.id===id?({...e,available:on}):e));renderEquipmentManager()}
function openEquipmentModal(id=''){
 const e=id?getEquipmentCatalog().find(x=>x.id===id):null;
 $('equipmentModalTitle').textContent=e?'Editar equipamento':'Adicionar equipamento';$('eqEditId').value=e?.id||'';$('eqName').value=e?.name||'';$('eqCategory').value=e?.category||'Outro';$('eqNotes').value=e?.notes||'';$('eqAvailable').checked=e?e.available:true;$('equipmentModal').classList.remove('hide');
}
function closeEquipmentModal(){$('equipmentModal').classList.add('hide')}
function slugId(name){return 'custom_'+name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'_').replace(/^_+|_+$/g,'')+'_'+Date.now().toString().slice(-5)}
function saveEquipment(){
 const name=$('eqName').value.trim();if(!name){alert('Digite o nome do equipamento.');return}
 const id=$('eqEditId').value;let list=getEquipmentCatalog();
 if(id)list=list.map(e=>e.id===id?({...e,name,category:$('eqCategory').value,notes:$('eqNotes').value.trim(),available:$('eqAvailable').checked}):e);
 else list.push({id:slugId(name),name,category:$('eqCategory').value,notes:$('eqNotes').value.trim(),available:$('eqAvailable').checked,system:false});
 saveEquipmentCatalog(list);closeEquipmentModal();renderEquipmentManager();
}
function deleteEquipment(id){const list=getEquipmentCatalog(),e=list.find(x=>x.id===id);if(!e)return;if(!confirm(`Excluir "${e.name}"? O histórico antigo será mantido.`))return;saveEquipmentCatalog(list.filter(x=>x.id!==id));renderEquipmentManager()}
function restoreDefaultEquipment(){if(!confirm('Restaurar a lista padrão? Equipamentos personalizados serão removidos.'))return;saveEquipmentCatalog(JSON.parse(JSON.stringify(defaultEquipmentCatalog)));renderEquipmentManager()}
...
function nav(id){
  ...
  if(id==='progress') renderProgress(); if(id==='settings') renderEquipmentManager();
}
...
document.addEventListener('DOMContentLoaded',()=>{
 exerciseOverrides=load('exerciseOverrides',{});
 document.querySelectorAll('nav button').forEach(b=>b.onclick=()=>nav(b.dataset.tab));
 $('day').onchange=buildWorkout;$('phase').onchange=buildWorkout; buildWorkout();renderDashboard();renderEquipmentManager();
